const { Appartement, Image } = require("../models");
...
let fileName;
exports.getAppartements = asyncWrapper(async (req, res) => {
  const data = await Appartement.findAll({ include: ["Images"] });
...
exports.createAppartement = asyncWrapper(async (req, res, next) => {
  ...
  const data = await Appartement.create(req.body);
